...
const PUZZLE_DB = [
  ...
  {e:["🚂","🎵","💑"],a:"dilwale dulhania le jayenge",h1:"DDLJ — iconic romance",h2:"Raj and Simran",h3:"Mustard fields of Punjab",cat:"Bollywood",diff:"hard"},
...
// 1. Input sanitizer — strips HTML tags, script injection, dangerous chars
...
// 2. Validate loaded puzzle structure from storage — prevent crash/XSS
const validatePuzzle = (data) => {
  if (!data || typeof data !== "object") return null;
  // emojis must be array of 1-4 non-empty strings
  if (!Array.isArray(data.emojis) || data.emojis.length < 1 || data.emojis.length > 4) return null;
  if (!data.emojis.every(e => typeof e === "string" && e.trim().length > 0)) return null;
  // answer must exist and be reasonable length
  if (typeof data.answer !== "string" || !data.answer.trim() || data.answer.length > 80) return null;
  // return sanitized version
  return {
    emojis:     data.emojis.map(e => e.trim().slice(0, 8)),
    answer:     sanitize(data.answer, 80).toLowerCase(),
    hint1:      sanitize(data.hint1 || "Think carefully!", 120),
    hint2:      sanitize(data.hint2 || "Almost there!", 120),
    hint3:      sanitize(data.hint3 || "Look carefully at the emojis!", 120),
    category:   sanitize(data.category || "Custom", 30),
    createdBy:  sanitize(data.createdBy || "Anonymous", 20),
    date:       typeof data.date === "string" ? data.date.slice(0, 10) : "",
  };
};

// 3. Validate leaderboard entry
const validateLBEntry = (data) => {
  if (!data || typeof data !== "object") return null;
  if (typeof data.name !== "string" || !data.name.trim()) return null;
  const score  = Math.min(Math.max(parseInt(data.score)  || 0, 0), 99999);  // cap 0–99999
  const streak = Math.min(Math.max(parseInt(data.streak) || 0, 0), 365);    // cap 0–365
  return {
    name:   sanitize(data.name, 20),
    score,
    streak,
    date:   typeof data.date === "string" ? data.date.slice(0, 10) : "",
  };
...
// 4. Safe storage wrapper — graceful fallback if window.storage unavailable
const safeStorage = {
  get:    async (key, shared) => { try { return await window.storage?.get(key, shared) || null;     } catch { return null; } },
  set:    async (key, val, shared) => { try { return await window.storage?.set(key, val, shared);   } catch { return null; } },
  list:   async (prefix, shared) => { try { return await window.storage?.list(prefix, shared) || { keys: [] }; } catch { return { keys: [] }; } },
};

// 5. Validate 6-char puzzle code
const isValidCode = (code) => /^[A-Z0-9]{6}$/.test(code);

// ─── PUZZLE ENGINE ────────────────────────────────────────────────────────────
class PuzzleEngine {
  constructor() { this.used = new Set(); }
  getByDiff(d) { return PUZZLE_DB.filter(p=>p.diff===d); }
  getByCat(c)  { return PUZZLE_DB.filter(p=>p.cat===c); }
  getRandom(diff="medium") {
    const pool=this.getByDiff(diff);
    const unused=pool.filter(p=>!this.used.has(p.a));
    if(!unused.length) this.used.clear();
    const src=unused.length?unused:pool;
    const pick=src[Math.floor(Math.random()*src.length)];
    this.used.add(pick.a);
    return {emojis:pick.e,answer:pick.a,hint1:pick.h1,hint2:pick.h2,hint3:pick.h3,category:pick.cat,difficulty:pick.diff};
  }
  getDaily() {
    const idx=Math.floor(Date.now()/86400000)%PUZZLE_DB.length;
    const p=PUZZLE_DB[idx];
    return {emojis:p.e,answer:p.a,hint1:p.h1,hint2:p.h2,hint3:p.h3,category:p.cat,difficulty:p.diff};
  }
  getTheme(cat,idx) {
    const pool=this.getByCat(cat);
    if(!pool.length) return this.getRandom();
    const p=pool[idx%pool.length];
    return {emojis:p.e,answer:p.a,hint1:p.h1,hint2:p.h2,hint3:p.h3,category:p.cat,difficulty:p.diff};
  }
  getStats() {
    const cats=[...new Set(PUZZLE_DB.map(p=>p.cat))];
    return {total:PUZZLE_DB.length,cats:cats.length,easy:PUZZLE_DB.filter(p=>p.diff==="easy").length,medium:PUZZLE_DB.filter(p=>p.diff==="medium").length,hard:PUZZLE_DB.filter(p=>p.diff==="hard").length};
  }
}
...
const S = {
  ...
  // ⚠️ input has NO animation — animation wrapper div pe hai
...
// ══════════════════════════════════════════════════════════════════════════════
// ─── ALL COMPONENTS OUTSIDE MAIN — YE HAI ASLI FIX ─────────────────────────
// Agar ye andar hote to har keypress pe re-create hote → keyboard close
// ══════════════════════════════════════════════════════════════════════════════

const DiffBadge = ({d}) => {
  const m={easy:{c:C.ok,bg:"rgba(52,211,153,0.12)"},medium:{c:"#f7c759",bg:"rgba(247,199,89,0.12)"},hard:{c:C.err,bg:"rgba(248,113,113,0.12)"}};
  const s=m[d]||m.medium;
  return <span style={{display:"inline-block",padding:"2px 8px",borderRadius:20,fontSize:9,fontWeight:700,textTransform:"uppercase",background:s.bg,color:s.c,marginLeft:5}}>{d}</span>;
};

// ⭐ YE HAI KEY FIX — GuessInput ek alag stable component hai
// Shake animation wrapper div pe hai, input pe nahi
// Is wajah se keyboard KABHI dismiss nahi hogi
...
const NameModal = ({nameInput, setNameInput, onSave, onSkip}) => (
  <div style={S.overlay}>
    <div style={S.modal}>
      ...
      <input
        style={{...S.input,textAlign:"left",padding:"11px 13px"}}
        placeholder="Your name..."
        value={nameInput}
        onChange={e=>setNameInput(e.target.value.slice(0,20))}   // ✅ max 20 chars
        maxLength={20}
        autoFocus
        autoComplete="off"
        autoCorrect="off"
        autoCapitalize="words"
        onKeyDown={e=>e.key==="Enter"&&nameInput.trim()&&onSave()}
      />
...
const TopBar = ({title, back, score, streak, soundOn, setSoundOn, lang, setLang, onBack}) => {
  ...
  return (
    <>
      <div style={S.topBar}>
        {back
          ? <button onClick={onBack} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",fontSize:18,padding:"0 2px",flexShrink:0}}>←</button>
...
export default function EmojiIQ() {
  ...
  // leaderboard — all entries validated before display
      ...
      const entries = await Promise.all(
        (res?.keys||[]).map(async k=>{
          try{
            ...
            if (!r) return null;
            return validateLBEntry(JSON.parse(r.value));  // ✅ validate each entry
          }catch{ return null; }
        })
      );
  ...
  const saveToLB = async(name, sc, st)=>{
    const safeName = sanitize(name, 20);                   // ✅ sanitize name
    const safeScore = Math.min(sc, 99999);                  // ✅ cap score
    const safeStreak = Math.min(st, 365);                   // ✅ cap streak
    if (!safeName) return;
    try{
  ...
  const handleWrong=()=>{
    setStreak(0); setFeedback({type:"err",msg:"Not quite! Try again 🤔"}); trigShake();
  };

  const doCheck=(p)=>{
    if(!guess.trim()||!p) return;
    guess.trim().toLowerCase()===p.answer.toLowerCase() ? handleCorrect(Math.max(100-hintsUsed*20,40)) : handleWrong();
  };
  ...
  const saveCustomPuzzle=async()=>{
    ...
    if(!cleanEmojis.length || !cleanAnswer) return;                     // ✅ validation
    const code=genCode();
    const data={
      emojis:   cleanEmojis.map(e=>e.trim().slice(0,8)),
      answer:   cleanAnswer,
      hint1:    sanitize(creatorHint1||"Think carefully!", 120),        // ✅ sanitize hints
      hint2:    sanitize(creatorHint2||"Almost there!", 120),
      hint3:    "Look carefully at the emojis!",                        // ✅ don't expose answer in hint3
      category: sanitize(creatorCategory||"Custom", 30),
      createdBy:sanitize(playerName||"Anonymous", 20),
      date:     getTodayKey(),
    };
    try{
      await safeStorage.set(`puzzle:${code}`,JSON.stringify(data),true);
      setCreatedCode(code); setCreatorStep("success"); play("correct"); boom();
  ...
  const loadCustomPuzzle=async()=>{
    ...
    if(!isValidCode(code)){setLoadError("Code must be exactly 6 characters (letters/numbers)!");return;} // ✅ strict code check
    try{
      const r = await safeStorage.get(`puzzle:${code}`, true);
      if(!r){setLoadError("Puzzle not found. Check the code!");return;}
      const parsed = validatePuzzle(JSON.parse(r.value));               // ✅ validate & sanitize
    ...
    <div style={S.wrap}>
      ...
      <div style={S.inner}>
        <div style={{textAlign:"center",marginBottom:16,paddingTop:8}}>
          ...
          <p style={{color:C.muted,margin:"3px 0 0",fontSize:11}}>{dbStats.total} puzzles · {dbStats.cats} categories · {LANGUAGES.length} languages · Offline</p>
        </div>
        ...
        <div style={{display:"flex",gap:9,marginBottom:9}}>
          <div style={S.homeBtn} onClick={()=>{setPuzzle(engine.getRandom(difficulty));resetPS();setScreen("practice")}}><span style={{fontSize:25}}>⚡</span><span style={{fontWeight:700,fontSize:13}}>Practice</span><span style={{color:C.muted,fontSize:10}}>Non-stop</span></div>
          <div style={S.homeBtn} onClick={()=>{const p=engine.getRandom(difficulty);setPuzzle(p);resetPS();setPlayers([{name:"Rahul",score:0,avatar:"🧑"},{name:"Priya",score:0,avatar:"👩"},{name:"You",score:0,avatar:"🎮",isYou:true}]);setMpRound(1);setMpTimer(30);setMpActive(true);setScreen("multiplayer");}}><span style={{fontSize:25}}>⚔️</span><span style={{fontWeight:700,fontSize:13}}>Multiplayer</span><span style={{color:C.muted,fontSize:10}}>Race friends</span></div>
        </div>
        ...
        <div style={{...S.card,padding:"11px 13px",marginBottom:9}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {Object.entries(THEMES).map(([name,thm])=>{
              ...
              return(
                <button key={name} onClick={()=>{setActiveTheme(name);setThemeIdx(0);setPuzzle(engine.getTheme(thm.cat,0));resetPS();setScreen("themes");}} style={{flex:"1 1 80px",padding:"9px 5px",borderRadius:13,border:"1.5px solid rgba(255,255,255,0.11)",background:"rgba(255,255,255,0.04)",cursor:"pointer",color:"#fff",textAlign:"center"}}>
                  <div style={{fontSize:20}}>{thm.icon}</div>
    ...
    <div style={S.wrap}><style>{BASE_CSS}</style>
      ...
      <div style={S.inner}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
          <span style={{color:C.muted,fontSize:11}}>Solved: {solvedCount}</span>
            ...
            <div style={{textAlign:"center"}}>
              ...
              <div style={{...S.card,marginTop:10,padding:"10px 13px",background:"rgba(200,121,255,0.06)"}}>
                ...
                <div style={{display:"flex",justifyContent:"center",gap:20}}>
                  <div><div style={{fontWeight:800,fontSize:17}}>{dailyHints}</div><div style={{fontSize:10,color:C.muted}}>Hints used</div></div>
        ...
        <div style={S.card}>
          ...
          {!mpActive?(
            <div style={{textAlign:"center"}}>
              ...
              <button style={{...S.btnMain,marginTop:10}} onClick={()=>{setMpRound(r=>r+1);setPuzzle(engine.getRandom(difficulty));setMpTimer(30);setMpActive(true);resetPS();}}>Next Round →</button>
            </div>
          ):(
            <>
